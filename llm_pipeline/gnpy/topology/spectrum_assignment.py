...
def build_oms_list(network: DiGraph, equipment: dict) -> List[OMS]:
    ...
    oms_vertices = [n for n in network.nodes() if isinstance(n, Roadm)] +\
                   [
                       ...
                       and (successor := next(network.successors(n), None)) is not None
                       and not isinstance(successor, Roadm)
                   ]
        ...
        for edge in network.edges([node]):
            if not isinstance(edge[1], Transceiver):
                ...
                while not isinstance(nd_out, Roadm):
                    ...
                    nd_out.oms = oms
                    n_temp = nd_out
                    nd_out = next((n[1] for n in network.edges([n_temp]) if n[1].uid != nd_in.uid), None)
                    if nd_out is None:
                        break
                    nd_in = n_temp
                ...
                if nd_out is not None:
                    oms.add_element(nd_out)
                    # nd_out is normally a Roadm, but some terminal chains may end earlier.
                    try:
