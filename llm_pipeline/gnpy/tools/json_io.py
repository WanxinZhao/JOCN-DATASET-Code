...
try:
    ...
    _YANG_IMPORT_ERROR = import_error
    ...
    def yang_to_legacy(json_data: dict) -> dict:
        ...
        if any(marker in json_data for marker in legacy_markers):
            return json_data
        raise ModuleNotFoundError(
            'oopt_gnpy_libyang is required to load YANG-formatted inputs.'
        ) from _YANG_IMPORT_ERROR

    def legacy_to_yang(json_data: dict) -> dict:
        raise ModuleNotFoundError(
            'oopt_gnpy_libyang is required to export YANG-formatted outputs.'
        ) from _YANG_IMPORT_ERROR
