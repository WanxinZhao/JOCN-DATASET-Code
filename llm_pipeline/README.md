...
## Local DT Web Console

This repository also includes a local Streamlit web console for running orchestration requests and browsing saved runs under `data/runs/`.

Install the extra dependency set:
...
- submit a natural-language request from the browser and run the orchestration pipeline
- inspect the captured execution log and final report in the same page
- browse available run folders
- inspect task and path summaries
- filter scenarios by destination, modulation, and status
- open per-scenario DSP plots and per-channel metrics
