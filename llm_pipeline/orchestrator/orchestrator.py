...
from utils.llm import llm_strict_mode
from utils.llm_audit import current_trace_dir, finish_llm_trace, start_llm_trace
...
def _save_run_artifacts(result: Dict[str, Any], base_dir: str = "data/runs") -> Dict[str, str]:
    ...
    return {
        ...
        "llm_trace_dir": str(run_dir / "llm_trace"),
    }
...
class Orchestrator:
    def __init__(self, max_iterations: int = 2) -> None:
        ...
        self._run_base_dir = Path(os.getenv("RUN_BASE_DIR", "data/runs"))
        self._log_dir = self._run_base_dir
        self._run_id = ""
        self._agent_log_paths: Dict[str, str] = {}
        configured_store = PhysicalLayerStore()
        topology_path = str(configured_store.topology_path)
        equipment_path = str(configured_store.equipment_path)
        ...
        self.executor = ExecutionAgent(
            log_hook=lambda message: self._log_agent("execution_agent", message),
            topology_path=topology_path,
            equipment_path=equipment_path,
        )
    ...
    def _start_run_logs(self) -> None:
        ...
        self._log_dir = self._run_base_dir / self._run_id / "agent_logs"
    ...
    def run(self, user_input: str) -> Dict[str, Any]:
        ...
        start_llm_trace(
            self._run_id,
            base_dir=self._run_base_dir,
            user_request=user_input,
            input_paths={
                "topology": self.physical_layer_db.topology_path,
                "equipment": self.physical_layer_db.equipment_path,
            },
        )
        try:
            return self._run_started(user_input)
        except Exception as exc:
            finish_llm_trace("failed", error=exc)
            raise

    def _run_started(self, user_input: str) -> Dict[str, Any]:
        ...
        result["artifacts"] = _save_run_artifacts(result, base_dir=str(self._run_base_dir))
        finish_llm_trace(
            "completed",
            extra={
        ...
        result["artifacts"]["llm_trace_dir"] = current_trace_dir()
